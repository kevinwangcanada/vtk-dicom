...
void vtkDICOMReader::SortFiles(vtkIntArray *files, vtkIntArray *frames)
{
  ...
  indexer->SetMetaData(this->MetaData);
  indexer->SetDesiredStackID(this->DesiredStackID);
  indexer->SetTimeAsVector(this->TimeAsVector);
  indexer->SetDesiredTimeIndex(this->DesiredTimeIndex);
  indexer->SetReverseSlices(this->MemoryRowOrder == vtkDICOMReader::BottomUp);

  indexer->Update();

  files->DeepCopy(indexer->GetFileIndexArray());
  frames->DeepCopy(indexer->GetFrameIndexArray());
  this->StackIDs->DeepCopy(indexer->GetStackIDs());

  // save the slice spacing and time information
  this->DataSpacing[2] = indexer->GetSliceSpacing();
  this->TimeDimension = indexer->GetTimeDimension();
  this->TimeSpacing = indexer->GetTimeSpacing();
}
