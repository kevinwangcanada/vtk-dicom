...
vtkStandardNewMacro(vtkDICOMIndexer);

//----------------------------------------------------------------------------
vtkDICOMIndexer::vtkDICOMIndexer()
{
...
//----------------------------------------------------------------------------
void vtkDICOMIndexer::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkDICOMIndexer::SetMetaData(vtkDICOMMetaData *meta)
{
...
//----------------------------------------------------------------------------
void vtkDICOMIndexer::SetDesiredStackID(const char *stackId)
{
  if (stackId == 0)
    {
    stackId = "";
...
struct vtkDICOMIndexerSortInfo
{
  int FileNumber;
  int FrameNumber;
  int InstanceNumber;
  ...
  static bool CompareInstance(
    const vtkDICOMIndexerSortInfo &si1, const vtkDICOMIndexerSortInfo &si2)
  {
...
const vtkDICOMValue& vtkDICOMIndexerGetFrameAttributeValue(
...
double vtkDICOMIndexerComputeLocation(
...
double vtkDICOMIndexerComputeSpacing(
  const std::vector<vtkDICOMIndexerSortInfo>& info,
  int *missing)
{
  double a = 0;
  ...
  while (m != 0)
    ...
    for (size_t i = 1; i < info.size(); i++)
      ...
      if (vtkDICOMIndexerSortInfo::CompareLocation(info[i-1], info[i]))
        {
        // compute the offset
...
//----------------------------------------------------------------------------
void vtkDICOMIndexer::SortFiles(vtkIntArray *files, vtkIntArray *frames)
{
  ...
  std::vector<vtkDICOMIndexerSortInfo> info;

  // sort by instance first
  for (int i = 0; i < numFiles; i++)
    {
    int inst = meta->GetAttributeValue(i, DC::InstanceNumber).AsInt();
    info.push_back(vtkDICOMIndexerSortInfo(i, inst));
    }
  std::stable_sort(info.begin(), info.end(),
    vtkDICOMIndexerSortInfo::CompareInstance);
  ...
  if (meta->HasAttribute(DC::SharedFunctionalGroupsSequence))
    ...
    std::vector<vtkDICOMIndexerSortInfo> stackInfo;
    ...
    for (int ii = 0; ii < numFiles; ii++)
      ...
      if (ii == 0 && numberOfFrames > 0)
        {
        firstStackId = vtkDICOMIndexerGetFrameAttributeValue(
      ...
      if (numberOfFrames > 0)
        ...
        if (vtkDICOMIndexerGetFrameAttributeValue(
        ...
        else if (vtkDICOMIndexerGetFrameAttributeValue(
      ...
      for (int k = 0; k < numberOfFrames; k++)
        ...
        if (timeTag.GetGroup() != 0)
          {
          t = vtkDICOMIndexerGetFrameAttributeValue(
        ...
        // get the StackID
        vtkDICOMValue stackId = vtkDICOMIndexerGetFrameAttributeValue(
        ...
        if (stackId.IsValid())
          ...
          position = vtkDICOMIndexerGetFrameAttributeValue(
            frameSeq, sharedSeq, k, DC::FrameContentSequence,
            DC::InStackPositionNumber).AsInt();
          }

        // check for valid Image Plane Module information
        vtkDICOMValue pv = vtkDICOMIndexerGetFrameAttributeValue(
          frameSeq, sharedSeq, k, DC::PlanePositionSequence,
          DC::ImagePositionPatient);
        vtkDICOMValue ov = vtkDICOMIndexerGetFrameAttributeValue(
        ...
        if (stackId == desiredStackId)
          ...
          double location = vtkDICOMIndexerComputeLocation(
            pv, ov, stackCheckNormal, &canSortStackByIPP);

          location /= spacingBetweenSlices;

          stackInfo.push_back(
            vtkDICOMIndexerSortInfo(i, k, inst, position, location, t));
          }
    ...
    for (int ii = 0; ii < numFiles; ii++)
      ...
      location = vtkDICOMIndexerComputeLocation(
        pv, ov, checkOrientation, &canSortByLocation);
      location /= spacingBetweenSlices;
      ...
      if (numberOfFrames <= 1)
        ...
        info.push_back(
          vtkDICOMIndexerSortInfo(i, 0, inst, position, location, t));
        }
      else
        {
        // multi-frame image
        ...
        if (timeSlotVector.IsValid() || sliceVector.IsValid())
          ...
          for (int k = 0; k < numberOfFrames; k++)
            ...
            info.push_back(
              vtkDICOMIndexerSortInfo(i, k, inst, position, frameloc, t));
            }
          }
        else
          {
          ...
          for (int k = 0; k < numberOfFrames; k++)
            ...
            info.push_back(
              vtkDICOMIndexerSortInfo(i, k, inst, inst, location, t));
            }
          }
        }
      }
  ...
  if (numSlices > 1)
    ...
    if (canSortByLocation)
      {
      std::stable_sort(info.begin(), info.end(),
        vtkDICOMIndexerSortInfo::CompareLocation);
      }
    else
      {
      std::stable_sort(info.begin(), info.end(),
        vtkDICOMIndexerSortInfo::ComparePosition);
      }

    // look for slices at the same location
    std::vector<vtkDICOMIndexerSortInfo>::iterator iter = info.begin();
    int slicesAtThisLocation = 0;
    while (iter != info.end())
      {
      std::vector<vtkDICOMIndexerSortInfo>::iterator nextIter = iter + 1;
      ...
      if (nextIter != info.end())
        ...
        if (canSortByLocation)
          {
          positionIncreased =
            vtkDICOMIndexerSortInfo::CompareLocation(*iter, *nextIter);
          }
        else
          {
          positionIncreased =
            vtkDICOMIndexerSortInfo::ComparePosition(*iter, *nextIter);
          }
        }
  ...
  if (canSortByLocation)
    ...
    if (locations > 1)
      {
      int missing = 0;
      locDiff = vtkDICOMIndexerComputeSpacing(info, &missing);
      if (missing > 0)
        {
